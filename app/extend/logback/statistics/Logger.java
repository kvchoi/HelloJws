...
import extend.logback.statistics.spi.StatisticsEvent;
...
public final class Logger implements org.slf4j.Logger, LocationAwareLogger,
        AppenderAttachable<IStatisticsEvent>, Serializable {
    ...
    /**
     * It is assumed that once the 'aai' variable is set to a non-null value, it will never be reset to null. it is further assumed that
     * only place where the 'aai'ariable is set is within the addAppender method. This method is synchronized on 'this' (Logger) protecting
     * against simultaneous re-configuration of this logger (a very unlikely scenario).
     * 
     * <p>
     * It is further assumed that the AppenderAttachableImpl is responsible for its internal synchronization and thread safety. Thus, we can
     * get away with *not* synchronizing on the 'aai' (check null/ read) because
     * <p>
     ...
     */
    transient private AppenderAttachableImpl<IStatisticsEvent> aai;
    /**
     ...
     * to <code>false</code> then the appenders located in the ancestors of this logger will not be used. However, the children of this
     * logger will inherit its appenders, unless the children have their additivity flag set to <code>false</code> too. See the user manual
     * for more details.
     */
    ...
    // details.
    public synchronized void addAppender(Appender<IStatisticsEvent> newAppender) {
        if (aai == null) {
            aai = new AppenderAttachableImpl<IStatisticsEvent>();
        }
    ...
    private int appendLoopOnAppenders(IStatisticsEvent event) {
        if (aai != null) {
    ...
    /**
     * Create a child of this logger by suffix, that is, the part of the name extending this logger. For example, if this logger is named
     * "x.y" and the lastPart is "z", then the created child logger will be named "x.y.z".
     * 
     * <p>
     * IMPORTANT: Calls to this method must be within a synchronized block on this logger.
     * 
     * @param lastPart the suffix (i.e. last part) of the child logger name. This parameter may not include dots, i.e. the logger separator
     *            character.
     * @return
     */
    ...
    Logger createChildByName(final String childName) {
        ...
        if (i_index != -1) {
            throw new IllegalArgumentException("For logger [" + this.name + "] child name ["
                    + childName + " passed as parameter, may not include '.' after index"
                    + (this.name.length() + 1));
        }
    ...
    /**
     * The next methods are not merged into one because of the time we gain by not creating a new Object[] with the params. This reduces the
     * cost of not logging by about 20 nanoseconds.
     */

    private void filterAndLog_0_Or3Plus(final String localFQCN, final Marker marker,
            final Level level, final String msg, final Object[] params, final Throwable t) {

        final FilterReply decision = loggerContext.getTurboFilterChainDecision_0_3OrMore(marker,
                this, level, msg, params, t);
    ...
    private void filterAndLog_1(final String localFQCN, final Marker marker, final Level level,
            final String msg, final Object param, final Throwable t) {

        final FilterReply decision = loggerContext.getTurboFilterChainDecision_1(marker, this,
                level, msg, param, t);
    ...
    private void filterAndLog_2(final String localFQCN, final Marker marker, final Level level,
            final String msg, final Object param1, final Object param2, final Throwable t) {

        final FilterReply decision = loggerContext.getTurboFilterChainDecision_2(marker, this,
                level, msg, param1, param2, t);
        ...
        buildLoggingEventAndAppend(localFQCN, marker, level, msg, new Object[] { param1, param2 },
                t);
    }

    private void buildLoggingEventAndAppend(final String localFQCN, final Marker marker,
            final Level level, final String msg, final Object[] params, final Throwable t) {
        IStatisticsEvent le = null;
        if (Level.DUMMY.equals(level)) {
            le = DummyStatisticsEvent.INSTANCE;
        } else {
            le = new StatisticsEvent(level.toString(), msg, params);
        }
    ...
    public void dummy() {
        filterAndLog_0_Or3Plus(FQCN, null, Level.DUMMY, null, null, null);
    }
    ...
    private FilterReply callTurboFilters(Marker marker, Level level) {
        return loggerContext.getTurboFilterChainDecision_0_3OrMore(marker, this, level, null, null,
                null);
    }
    ...
    /**
     * After serialization, the logger instance does not know its LoggerContext. The best we can do here, is to return a logger with the
     * same name returned by org.slf4j.LoggerFactory.
     * 
